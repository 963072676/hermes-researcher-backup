...
# 已封装的安全 ingest 函数 (call site 已经 fail-fast, 内部 POST 不需重复送 context)
# 排除名单: 函数 / 装饰器 名含这些字符串的, lint 跳过
GUARDED_INGEST_FUNCTIONS = [
    "guarded_ingest",
    "ingest_to_kb",  # memory_slim 系列
]
...
def lint_ingest_call(code: str, filepath: str) -> list[dict]:
    ...
    排除: 如果整个块在 GUARDED_INGEST_FUNCTIONS 列出的函数体内, 跳过 (call site 已校验).
    """
    issues = []
    ...
    for i, line in enumerate(lines):
        ...
        if "data=json.dumps" not in window:
            continue
        # 检查是否在 GUARDED_INGEST_FUNCTIONS 函数体内 (向上找 30 行)
        func_start = max(0, i - 30)
        above = chr(10).join(lines[func_start:i])
        in_guarded = False
        for guarded in GUARDED_INGEST_FUNCTIONS:
            # 看有没有 "def <guarded>(" 出现在 above 块中
            if f"def {guarded}(" in above:
                in_guarded = True
                break
        if in_guarded:
            continue  # 跳过, call site 会校验
